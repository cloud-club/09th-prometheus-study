...
계속하여 oom으로 kill 하였다. 프로메테우스의 metric 수집 타임스탬프 차이는 15~30초 주기이기 때문에  
...
180번의 oom이 발생하고 있었다. 이는 즉, 메모리 점유율 모니터링만 맹신할 수 없음을 의미한다.
...
처음에는 컨테이너는 호스트의 커널을 공유하고 있다는 것을 입증해보자는 취지에서 시작한 실습이지만 그걸 넘어서 더 많은 것을 깨달을 수 있었던 것 같다.
